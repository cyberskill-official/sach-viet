...
export function OrderHistory({ highlightId }: { highlightId?: string } = {}) {
  ...
  return (
    <LuxuryShell width="xl" tourId="tour.orders">
      <p className="sv-lux-eyebrow">{t("nav.account")}</p>
      <h1 className="sv-font-display mt-2 text-4xl tracking-tight" data-tour="orders-heading">{t("orders.title")}</h1>
      ...
      {!loading && !error && orders.length === 0 ? (
        <div className="sv-glass-card cs-empty-state mt-8 rounded-2xl p-10" data-tour="orders-list">
          <h2 className="sv-font-display text-2xl">{t("orders.empty")}</h2>
          <Link className="cs-button mt-6 min-h-11" href="/">{t("nav.home")}</Link>
        </div>
      ) : null}
      <div className="mt-8 space-y-4" data-tour="orders-list">
        {orders.map((order) => (
          <article
            className={`sv-glass-card flex flex-wrap items-center justify-between gap-5 rounded-2xl p-6${highlightId === order.id ? " ring-2 ring-accent-strong" : ""}`}
            key={order.id}
          >
            <div>
