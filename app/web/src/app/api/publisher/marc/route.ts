  ...
  try {
    ...
    try {
      return NextResponse.json({
        marcRecords: await listPublisherMarcRecords(store, session.user, { publisherId }),
      });
    } finally {
      await store.close();
    }
...
export async function POST(request: Request) {
  try {
    ...
    try {
      return NextResponse.json(
        { marcRecord: await registerPublisherMarcRecord(store, session.user, await request.json()) },
        { status: 201 },
      );
    } finally {
      await store.close();
