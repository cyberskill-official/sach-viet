...
export async function POST(request: Request) {
  ...
  let session;
  try {
    session = readSession(getAuthStore(), token, process.env.AUTH_SESSION_SECRET);
  } catch {
  ...
  try {
    ...
    const checkout =
      provider === "paypal"
        ? await createPayPalCheckoutOrder(store, order.id)
        : await createStripeCheckoutSession(store, order.id);
