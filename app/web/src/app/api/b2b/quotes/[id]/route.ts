...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    try {
      return NextResponse.json({ quote: await getStaffQuote(store, auth.user, id) });
    } finally {
      await store.close();
    }
  } catch (error) {
  ...
  try {
    ...
    try {
      if (Array.isArray(body?.items)) {
        return NextResponse.json({ quote: await setQuoteItemPrices(store, auth.user, { quoteId: id, items: body.items }) });
      }
      return NextResponse.json({ quote: await transitionQuoteStatus(store, auth.user, { quoteId: id, status: body?.status }) });
    } finally {
      await store.close();
    }
  } catch (error) {
