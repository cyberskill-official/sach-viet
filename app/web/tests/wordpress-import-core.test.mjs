...
function fixture(run) {
  ...
  try {
    return run({ authStore, commerceStore, events, dbPath });
  } finally {
    authStore.close();
    commerceStore.close();
...
test("PHPass hashes verify and scrypt path stays intact", () => {
...
test("dry_run records outcomes without mutating users or orders", () =>
  fixture(({ authStore, commerceStore }) => {
    const payload = sampleFixture();
    const result = importWordpressFixture(authStore, commerceStore, payload, { mode: "dry_run" });
    assert.equal(result.acceptedCount, 2);
    assert.equal(result.unmatchedCount, 1);
    assert.equal(authStore.db.prepare("SELECT COUNT(*) AS c FROM users").get().c, 0);
    assert.equal(commerceStore.db.prepare("SELECT COUNT(*) AS c FROM orders").get().c, 0);
    const outcomes = listWordpressImportOutcomes(authStore, result.runId);
    assert.ok(outcomes.some((row) => row.outcome === "unmatched"));
  }));

test("apply imports users/orders with legacy ids, PHPass login, and idempotent re-import", () =>
  fixture(({ authStore, commerceStore }) => {
    const payload = sampleFixture();
    const first = importWordpressFixture(authStore, commerceStore, payload, { mode: "apply" });
    assert.equal(first.acceptedCount, 2);
    assert.equal(first.unmatchedCount, 1);
    const user = authStore.db.prepare("SELECT legacy_wp_user_id AS legacyId, password_hash AS hash FROM users WHERE email = ?").get("buyer@example.test");
    assert.equal(user.legacyId, "wp-user-1");
    assert.ok(user.hash.startsWith("$P$"));
    const loginResult = login(authStore, {
      email: "buyer@example.test",
      password: "imported-password-ok",
      sessionSecret,
    });
    assert.equal(loginResult.ok, true);
    const order = commerceStore.db
    ...
    const second = importWordpressFixture(authStore, commerceStore, payload, { mode: "apply" });
    assert.equal(second.skippedCount, 2);
    assert.equal(authStore.db.prepare("SELECT COUNT(*) AS c FROM users").get().c, 1);
    assert.equal(commerceStore.db.prepare("SELECT COUNT(*) AS c FROM orders").get().c, 1);
  }));

test("admin status and apply gate non-admins", () =>
  fixture(({ authStore, commerceStore }) => {
    assert.throws(() => getWordpressImportStatus(authStore, { role: "customer" }), /Admin access/);
    assert.throws(
      () => applyWordpressImportAsAdmin(authStore, commerceStore, { role: "vendor" }, sampleFixture()),
      /Admin access/,
    );
    const status = getWordpressImportStatus(authStore, { role: "admin" });
    assert.equal(status.adapter, "fixture");
    assert.equal(status.mysqlClient, false);
    assert.equal(status.wordpressPhpRuntime, false);
    const result = applyWordpressImportAsAdmin(authStore, commerceStore, { role: "admin" }, sampleFixture(), {
      mode: "dry_run",
    });
