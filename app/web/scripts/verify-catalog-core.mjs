...
const productsSchema = migration.match(/CREATE TABLE IF NOT EXISTS products \(([\s\S]*?)\);/)?.[1] ?? "";

if (!migration.includes("CREATE TABLE IF NOT EXISTS vendor_offers")) throw new Error("Vendor offers schema is missing.");
...
if (!source.includes("canWriteVendorOffer")) throw new Error("Vendor offer ownership enforcement is missing.");
if (!source.includes("export async function listCategories")) throw new Error("Category listing helper is missing.");
