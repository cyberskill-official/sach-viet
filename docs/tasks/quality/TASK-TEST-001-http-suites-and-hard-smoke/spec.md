---
id: TASK-TEST-001
title: "HTTP handler tests and hard production smoke gate"
template: task@1
type: improvement
module: quality
author: "@cursor"
...
Add handler-imported HTTP tests for register/checkout. smoke:production hard-fails health, catalog, login, and checkout path; skipped required checks fail. Restore uses --exit-on-error. <!-- authority: llm-explicit -->
...
Routes and authz: scripts/smoke-production.mjs; scripts/restore-postgres.mjs. <!-- authority: llm-explicit -->
...
- Add handler-imported HTTP tests for register/checkout. smoke:production hard-fails health, catalog, login, and checkout path; skipped required checks fail. Restore uses --exit-on-error. <!-- authority: llm-explicit -->
- Tests: tests/auth-http.test.mjs, tests/commerce-http.test.mjs, tests/smoke-production.test.mjs, tests/backup-restore.test.mjs. <!-- authority: llm-explicit -->
