---
id: TASK-DATA-001
title: "Replace spawnSync DB adapter with async pg transactions"
template: task@1
type: improvement
module: data
author: "@cursor"
...
Use an in-process async pg Pool with real BEGIN/COMMIT/ROLLBACK on Vercel, Docker, and CI. Stop runtime public-schema DDL on Vercel. Verify TLS except documented local override. Remove synckit workers. <!-- authority: llm-explicit -->
...
Routes and authz: All store factories via openDatabase. GET /api/health uses getSharedPool + sslOptionsForDatabaseUrl. <!-- authority: llm-explicit -->
...
- Use an in-process async pg Pool with real BEGIN/COMMIT/ROLLBACK on Vercel, Docker, and CI. Stop runtime public-schema DDL on Vercel. Verify TLS except documented local override. Remove synckit workers. <!-- authority: llm-explicit -->
- Tests: tests/pg-adapter-failure.test.mjs, tests/pg-concurrency.test.mjs, tests/backup-restore.test.mjs. <!-- authority: llm-explicit -->
