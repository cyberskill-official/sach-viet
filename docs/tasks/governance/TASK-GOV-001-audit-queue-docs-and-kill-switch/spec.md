---
id: TASK-GOV-001
title: "Author audit task queue, honest docs, and commerce kill-switch"
template: task@1
type: improvement
module: governance
author: "@cursor"
...
Publish the post-audit CyberOS queue, align README/ops/cutover docs with Phase A live but not real-commerce-ready, and ship COMMERCE_MUTATIONS_ENABLED as a deployable freeze for checkout, payouts, and WordPress apply. <!-- authority: llm-explicit -->
...
Routes and authz: Flag is read by POST /api/checkout, POST /api/admin/payouts, POST /api/admin/wordpress-import/apply. Unset or any value other than 0/false/off allows mutations so Production stays up until an operator deploys COMMERCE_MUTATIONS_ENABLED=0. <!-- authority: llm-explicit -->
...
- Publish the post-audit CyberOS queue, align README/ops/cutover docs with Phase A live but not real-commerce-ready, and ship COMMERCE_MUTATIONS_ENABLED as a deployable freeze for checkout, payouts, and WordPress apply. <!-- authority: llm-explicit -->
- Tests: tests/commerce-http.test.mjs (handler status codes, not source greps). <!-- authority: llm-explicit -->
