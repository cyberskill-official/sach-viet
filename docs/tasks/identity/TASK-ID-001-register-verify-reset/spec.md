---
id: TASK-ID-001
title: "Customer register, email verify, and password reset"
template: task@1
type: feature
module: identity
author: "@cursor"
...
Customers can register, verify email, and reset passwords. Login refuses unverified accounts. Sessions are HMAC-validated in proxy. Wrong role still reaches portal pages that render /forbidden; missing/invalid cookie redirects to /login?redirect=. <!-- authority: llm-explicit -->
...
Routes and authz: POST /api/auth/register, GET|POST /api/auth/verify, POST /api/auth/forgot, POST /api/auth/reset, POST /api/auth/login. Pages /register /forgot /reset /login. <!-- authority: llm-explicit -->
...
- Customers can register, verify email, and reset passwords. Login refuses unverified accounts. Sessions are HMAC-validated in proxy. Wrong role still reaches portal pages that render /forbidden; missing/invalid cookie redirects to /login?redirect=. <!-- authority: llm-explicit -->
- Tests: tests/auth-http.test.mjs and tests/auth-core.test.mjs (handler + core, not greps). <!-- authority: llm-explicit -->
