...
**Project:** SachViet Luxury
**Generated:** 2026-08-23 22:08:00
...
Uses `data-theme="dark"` on `<html>` (via `ThemeProvider` + `sv_theme` localStorage).

| Token / utility | Dark behavior |
|-----------------|---------------|
| `--sv-lux-glass-border` | Subtle white edge (14% mix) for gold-on-dark glass |
| `--sv-lux-glass-surface` / `--sv-lux-glass-heavy` | Higher panel opacity on dark stone backgrounds |
| `.sv-lux-hero-glow` | Stone-950 base with restrained gold radial |
| `.sv-lux-eyebrow` | Gold-soft tint for ≥4.5:1 contrast on dark panels |
| Glass shadows | Deeper scrim (`black 28%`) instead of light lift |

CyberOS design tokens (`--cs-color-*`) continue to follow `data-theme` from `@cyberskill/design`.
